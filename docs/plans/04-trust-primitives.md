...
- **The user-facing command shipped as `avc verify`, not `avc fsck`.** "fsck" is a Unix-ism at odds with AVC's "agents and non-technical users alike" audience; `verify` says what it does. `fsck` remains as a cobra alias for git/unix muscle memory, and the internal package keeps the technical name.
